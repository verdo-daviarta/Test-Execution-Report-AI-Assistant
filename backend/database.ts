...
import { migrateAuthSchema } from './auth/schema';
...
export function openDatabase(databasePath = path.join(process.cwd(), 'data', 'test-execution-report.db')) {
  ...
  // Additive migration: keep old rows/data and mark them as generated by default.
  for (const table of ['test_cases', 'project_test_cases']) {
    const columns = db.prepare(`PRAGMA table_info(${table})`).all() as Array<{ name: string }>;
    if (!columns.some(column => column.name === 'is_manual')) {
      db.exec(`ALTER TABLE ${table} ADD COLUMN is_manual INTEGER NOT NULL DEFAULT 0`);
    }
  }
  return db;
}
